...
### 单应性矩阵H
单应性矩阵（Homography Matrix） 是投影几何中的核心概念，用于描述两个平面之间的透视变换关系。在计算机视觉中，它常用于将一个平面上的点映射到另一个平面，例如将真实世界的平面投影到图像平面，或在不同视角的图像之间进行转换。标定时我们采用了许多不同角度对于同一物体的拍摄图像，单应性矩阵是构建不同图像中点与点之间映射关系的关键工具。
![](H.png)
理论上，求解单应性矩阵H需要4个非共线的拍摄点对。
## 张氏标定法的步骤
